...
namespace Fibula.Server.Notifications
{
    ...
    public class PlayerCancelWalkNotification : Notification
    {
        ...
        public PlayerCancelWalkNotification(Func<IEnumerable<IPlayer>> findTargetPlayersFunc, IPlayer player)
        ...
        public override bool Post(INotificationContext context, IPlayer player)
        {
            ...
            var packet = new PlayerCancelWalk()
            {
                LastDirection = (uint)player.Direction,
            };

            return targetBuffer.Post(new GameNotification() { PlayerCancelWalk = packet });
        }
    }
}
