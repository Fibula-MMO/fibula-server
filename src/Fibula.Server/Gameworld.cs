// -----------------------------------------------------------------
// <copyright file="Gameworld.cs" company="2Dudes">
...
namespace Fibula.Server.Mechanics
{
    ...
    public class Gameworld : IGameworld
    {
            ...
            IScheduler scheduler)
        {
            ...
            {
                Id = this.applicationContext.Options.World.Name,
            ...
            this.containerManager.ContainerOpened += this.OnContainerOpened;
            this.containerManager.ContainerClosed += this.OnContainerClosed;
            this.containerManager.ItemAdded += this.OnItemAddedToContainer;
            this.containerManager.ItemUpdated += this.OnItemUpdatedInContainer;
            this.containerManager.ItemRemoved += this.OnItemRemovedFromContainer;
        }

        /// <summary>
        /// Event fired when there is a notification ready to be broadcasted to the <see cref="IGameworld"/> subscribers.
        /// </summary>
        public event GameNotificationReadyHandler NotificationReady;
        ...
        public void SendNotification(INotification notification)
        {
            ...
            this.NotificationReady?.Invoke(notification);
        }
        ...
        public void AddOrAggregateCondition(IThing thing, ICondition condition, TimeSpan duration)
        {
            ...
            if (!thing.TrackedEvents.TryGetValue(condition.EventType, out IEvent existingEvent))
            {
                ...
                {
                    // scheduleAsync: true
                    // this.NotificationReady?.Invoke(new GenericNotification(() => player.YieldSingleItem(), new PlayerConditionsPacket(player)));
                    this.SendNotification(new PlayerConditionsUpdateNotification(() => player.YieldSingleItem(), player));
                }
        ...
        private void OnContainerClosed(IPlayer forPlayer, byte atIndex)
        {
            var notification = new ContainerClosedNotification(() => forPlayer.YieldSingleItem(), atIndex);

            this.SendNotification(notification);
        }

        private void OnContainerOpened(IPlayer forPlayer, byte atIndex, IContainerItem container)
        {
            var notification = new ContainerOpenNotification(() => forPlayer.YieldSingleItem(), atIndex, container);

            this.SendNotification(notification);
        }

        private void OnItemRemovedFromContainer(IPlayer forPlayer, byte fromContainerId, byte atIndex)
        {
            var notification = new ContainerItemRemovedNotification(() => forPlayer.YieldSingleItem(), fromContainerId, atIndex);

            this.SendNotification(notification);
        }

        private void OnItemUpdatedInContainer(IPlayer forPlayer, byte atIndex, IItem item)
        {
            var notification = new ContainerItemUpdatedNotification(() => forPlayer.YieldSingleItem(), atIndex, item);

            this.SendNotification(notification);
        }

        private void OnItemAddedToContainer(IPlayer forPlayer, byte atIndex, IItem item)
        {
            var notification = new ContainerItemAddedNotification(() => forPlayer.YieldSingleItem(), atIndex, item);
        ...
        private void SkilledCreatureSkillChanged(ICreatureWithSkills skilledCreature, ISkill skillThatChanged, uint previousLevel, byte previousPercent, long? countDelta = null)
        {
            ...
            if (skillThatChanged.Percent != previousPercent || skillThatChanged.Type == SkillType.Experience)
            {
                ...
                // var playerSkillsUpdateNotification = new PlayerSkillsUpdateNotification(() => player.YieldSingleItem(), player);
                var playerStatsUpdateNotification = new PlayerStatsUpdateNotification(() => player.YieldSingleItem(), player);
        ...
        private void CreatureStatChanged(ICreature creature, IStat statThatChanged, uint previousValue, byte previousPercent)
        {
            ...
            if (creature is IPlayer player)
            {
                ...
                {
                    this.SendNotification(new CreatureSpeedChangeNotification(() => player.YieldSingleItem(), player));
                }

                // And this updates the health of the player.
                this.SendNotification(new PlayerStatsUpdateNotification(() => player.YieldSingleItem(), player));
            }
        }
        ...
        private void MiscellaneousEventsLoop(object tokenState)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    var notification = new WorldLightChangedNotification(() => this.creatureManager.FindAllPlayers(), this.worldInfo.LightLevel, this.worldInfo.LightColor);
