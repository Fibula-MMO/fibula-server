...
namespace Fibula.Server.Mechanics.Handlers
{
    ...
    public sealed class GameLogInHandler : GameHandler
    {
        ...
        public override IEnumerable<IOutboundPacket> HandleRequest(IIncomingPacket incomingPacket, IClient client)
        {
            ...
            if (!(unitOfWork.Accounts.FindByNumber(loginInfo.AccountNumber) is AccountEntity account && account.Password.Equals(loginInfo.Password)))
            {
                // TODO: hardcoded messages.
                return new GameServerDisconnectPacket("The account number and password combination is invalid.").YieldSingleItem();
            }

            if (!(unitOfWork.Characters.FindByName(loginInfo.CharacterName) is CharacterEntity character))
            {
                // TODO: hardcoded messages.
