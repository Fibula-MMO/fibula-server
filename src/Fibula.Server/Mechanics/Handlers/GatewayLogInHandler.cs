...
namespace Fibula.Server.Mechanics.Handlers
{
    ...
    public sealed class GatewayLogInHandler : BaseRequestHandler
    {
        ...
        public override IEnumerable<IOutboundPacket> HandleRequest(IIncomingPacket incomingPacket, IClient client)
        {
            ...
            {
                this.Logger.LogInformation($"Client attempted to connect with version: {accountLoginInfo.ClientVersion}, OS: {accountLoginInfo.ClientOs}. Expected version: {this.ApplicationContext.Options.SupportedClientVersion.Numeric}.");
            ...
            if (!uint.TryParse(accountLoginInfo.AccountName, out uint accountNumber) || !(unitOfWork.Accounts.FindByNumber(accountNumber) is AccountEntity account && account.Password.Equals(accountLoginInfo.Password)))
            {
                // TODO: hardcoded messages.
